...
st.caption("Build: 2026-09-07 • Widget IDs hardened for Streamlit Cloud")

with st.sidebar:
    st.header("1. Upload data")
    uploaded = st.file_uploader(
        "Excel/CSV", type=["xlsx", "xlsm", "xls", "csv"], key="upload_data_file"
    )
    st.divider()
    st.header("2. Analysis controls")
    if st.button("🔧 Reset analysis session", key="reset_analysis_session", use_container_width=True):
...
selected = st.selectbox("Worksheet", sheet_names, key="worksheet_selector")
...
with tabs[2]:
    ...
    if not dates or not numeric:
        ...
        dc = st.selectbox("Date", dates, key="trend_date")
        vc = st.selectbox(
            "Measure",
            numeric,
            index=numeric.index(business["sales"]) if business["sales"] in numeric else 0,
            key="trend_measure",
        )
        freq = st.selectbox("Period", ["M", "W", "D"], index=0, key="trend_period")
...
with tabs[3]:
    ...
    if not cats or not numeric:
        ...
        cat = st.selectbox("Dimension", cats, key="segment_dimension")
        metric = st.selectbox(
            "Metric",
            numeric,
            index=numeric.index(business["sales"]) if business["sales"] in numeric else 0,
        ...
        top_n = st.slider("Top N", 5, 30, 10, key="segment_top_n")
...
with tabs[6]:
    ...
    q = st.text_input(
        ...
        key="ask_aryan_business_question",
        placeholder="e.g. top products by sales / total profit / show missing values",
    )
    if q:
...
with tabs[7]:
    ...
    st.download_button(
        "⬇️ Download cleaned CSV", clean_csv, file_name="aryan_cleaned_data.csv",
        mime="text/csv", key="download_cleaned_csv"
    )
    ...
    st.download_button(
        "⬇️ Download DAX measures", dax, file_name="aryan_powerbi_measures.txt",
        mime="text/plain", key="download_dax_measures"
    )
...
"""
    st.download_button(
        "⬇️ Download Power Query starter", pq, file_name="aryan_powerquery_starter.txt",
        mime="text/plain", key="download_powerquery"
    )
...
with tabs[8]:
    ...
    report_text = "\n".join(report)
    st.download_button(
        "⬇️ Download executive Markdown report", report_text, file_name="aryan_executive_report.md",
        mime="text/markdown", key="download_markdown_report"
    )
    pdf_lines = report_text.split("\n")
    pdf_bytes = build_pdf_report("Aryan Data Analyst — Executive Report", pdf_lines)
    st.download_button(
        "⬇️ Download executive PDF report", pdf_bytes, file_name="aryan_executive_report.pdf",
